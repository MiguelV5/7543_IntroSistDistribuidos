...
class StopAndWait():
    ...
    def send_segment(self, packet):
        ...
        while True:
            ...
            try:
                ackPacket, _ = self.socket.recvfrom(self.mss + len(self.header))
                # TODO: check if the acknowledgement is from the correct address
                logging.info("Received acknowledgement from: {}".format(self.addr)) 
                # check if data has been acknowledged
                sq, ackNum, _ = self.decodePacket(ackPacket)
                logging.info("Acknowledgement sequence: {} acknowledgement number: {}".format(sq, ackNum))
                print("ackNum: {} sq: {} self.sq: {}".format(ackNum, sq, self.seq))
                if ackNum == 1 and sq == self.seq:
                    if self.debug:
                        logging.info("Chunk acknowledged sequence: {}".format(self.seq))
                ...
                if self.debug:
                    logging.info("Timeout reached, resending chunk for seq: {}".format(self.seq))
                # If the acknowledgment is not received within the timeout period, resend the packet
                # resend data
    ...
    def receive(self, data, addr):
        ...
        logging.info("Received packet sequence: {} from addr: {}".format(sq, addr))
        if len(decodedData) == 0:
            # end of transmission
            self.send_ack(sq, addr)
            self.last_seq = sq
            return b''
        if self.last_seq != None and sq == self.last_seq:
            # we have received a duplicate packet
            if self.debug:
                logging("Received duplicate packet sequence: {}".format(self.seq))
